...
- [eventlog-server-unencrypted.md](eventlog-server-unencrypted.md) - Simplify `EventLogServerUnencrypted` so `Storage` is the only event / entry storage service, removing the runtime `EventJournal` dependency while preserving existing server behavior.
- [eventlog-server-unencrypted-multi-replica.md](eventlog-server-unencrypted-multi-replica.md) - Redesign `EventLogServerUnencrypted` for multi-replica deployments so writes process handlers inside a store transaction and only persist entries after successful processing.

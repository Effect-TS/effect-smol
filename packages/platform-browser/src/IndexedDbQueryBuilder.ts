...
export declare namespace IndexedDbQuery {
  ...
  export type ModifyWithKey<Table extends IndexedDbTable.AnyWithProps> =
    SourceTableModifySchemaType<Table>;
...
const getReadSchema = (from: IndexedDbQuery.From<any>) => {
...
>(options: {
  ...
  const self = Object.create(CommonProto);
  self.table = options.table;
  self.database = options.database;
  self.IDBKeyRange = options.IDBKeyRange;
  self.transaction = options.transaction;
  self.select = <Index extends IndexedDbDatabase.IndexFromTable<Table>>(
    index?: Index,
  ) =>
    selectMakeProto({
      from: self,
  ...
  self.delete = <Index extends IndexedDbDatabase.IndexFromTable<Table>>(
    index?: Index,
  ) =>
  ...
  self.count = <Index extends IndexedDbDatabase.IndexFromTable<Table>>(
    index?: Index,
  ) =>
    countMakeProto({
      from: self,
  ...
  self.insert = (value: any) =>
    modifyMakeProto({ from: self, value, operation: "add" });

  self.upsert = (value: any) =>
    modifyMakeProto({ from: self, value, operation: "put" });

  self.insertAll = (values: Array<any>) =>
    modifyAllMakeProto({ from: self, values, operation: "add" });

  self.upsertAll = (values: Array<any>) =>
    modifyAllMakeProto({ from: self, values, operation: "put" });
...
>(options: {
  ...
  const limit = (limit: number): IndexedDbQuery.Delete<Table, Index> =>
    deleteMakeProto({ delete: self as any, limitValue: limit });

  const equals = (
    value: IndexedDbQuery.ExtractIndexType<Table, Index>,
  ): IndexedDbQuery.Delete<Table, Index> =>
    deleteMakeProto({ delete: self as any, only: value });
...
const modifyMakeProto = <Table extends IndexedDbTable.AnyWithProps>(options: {
  readonly from: IndexedDbQuery.From<Table>;
  readonly value: IndexedDbTable.TableSchema<Table>["Type"];
  readonly operation: "add" | "put";
}): IndexedDbQuery.Modify<Table> => {
  const self: any = Effect.suspend(() =>
    applyModify({ query: self, value: options.value }),
  );
  self.from = options.from;

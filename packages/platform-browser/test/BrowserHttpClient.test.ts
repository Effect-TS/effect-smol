...
describe("BrowserHttpClient", () => {
  it.effect("json", () =>
    Effect.gen(function* () {
      const body = yield* HttpClient.get("http://localhost:8080/my/url").pipe(Effect.flatMap((_) => _.json))
  ...
  it.effect("stream", () =>
    Effect.gen(function* () {
      const body = yield* HttpClient.get("http://localhost:8080/my/url").pipe(
        Effect.flatMap((response) => response.stream.pipe(Stream.decodeText(), Stream.mkString))
      )
      assert.deepStrictEqual(body, '{ "message": "Success!" }')
    }).pipe(
      Effect.provide(

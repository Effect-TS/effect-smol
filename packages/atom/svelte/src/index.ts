/**
 * Svelte bindings for Effect Atoms. These functions read and write atoms
 * through the current `RegistryContext`, expose reactive read handles whose
 * `current` getter participates in Svelte reactivity, keep atoms mounted for
 * cleanup, and read `AtomRef` values.
 *
 ...
 *
 * Every function calls `injectRegistry()` (and the mounting ones call
 * `onDestroy`), so each must be called during component initialisation — the
 * same rule as Svelte's own `getContext`.
 *
 * @since 4.0.0
 */
 ...
 * `<input bind:value={bindable.current} />`. It mirrors a `$bindable` prop
 * rather than `$state`.
 *
 * @category models
 * @since 4.0.0
 */
export interface AtomBindable<A> {
  current: A
}
 ...
 * a two-way {@link AtomBindable} handle. Assigning to `current` writes the atom,
 * so the handle can be bound directly with `bind:`, e.g.
 * `<input bind:value={bindable.current} />`.
 *
 * @category bindings
 * @since 4.0.0
 */
export const useAtomBindable = <A>(atom: () => Atom.Writable<A, A>): AtomBindable<A> => {
  const registry = injectRegistry()
  const value = reactiveValue(atom, (a) => registry.get(a), subscribeAtom(registry))
  return {
    get current() {
 ...
 * value, or rejects with the squashed failure cause. Read `current` from an
 * `{#await}` block — or, with the experimental async feature enabled, `await` it
 * inside a `<svelte:boundary>` whose `pending` snippet renders the loading
 * state. The promise identity changes when the result changes, so the awaiting
 * block re-runs on every transition.
 *
 * @category bindings
 * @since 4.0.0
 */
export const useAtomPromise = <A, E>(

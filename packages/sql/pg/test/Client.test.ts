...
it.layer(PgContainer.layerClient, { timeout: "30 seconds" })("PgClient", (it) => {
  ...
  it.effect("stream", () =>
    Effect.gen(function* () {
      ...
      expect(rows).toEqual([{ generate_series: 1 }, { generate_series: 2 }, { generate_series: 3 }])
    })
  )
})
...
it.layer(PgContainer.layerClientWithTransforms, { timeout: "30 seconds" })("PgClient transforms", (it) => {
  ...
  it.effect("interruption", () =>
    Effect.gen(function* () {
      ...
      yield* conn.executeRaw("select pg_sleep(1000)", []).pipe(Effect.timeoutOption("50 millis"), TestClock.withLive)

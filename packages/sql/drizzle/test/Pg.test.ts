...
class ORM extends ServiceMap.Service<ORM>()("ORM", { make: Pg.make({ schema: { users } }) }) {
  static Client = Layer.effect(this, this.make).pipe(Layer.provideMerge(PgContainer.layerClient));
}
...
describe.sequential("Pg", () => {
  ...
  it.effect(
    "fail properly for conflict issues",
    () => {
      ...
      return Effect.gen(function* () {
        ...
        yield* Effect.flip(db.insert(users).values({ name: "Alice", snakeCase: "test" }));

...
describe("SqlEventLogServer", () => {
  it.effect("persists remote id across storage instances", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const storageA = yield* SqlEventLogServer.makeStorage().pipe(Effect.provideService(SqlClient.SqlClient, sql))
        const storageB = yield* SqlEventLogServer.makeStorage().pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.effect("writes entries and streams changes", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const sql = yield* SqliteClient.make({ filename: ":memory:" })
        const storage = yield* SqlEventLogServer.makeStorage().pipe(Effect.provideService(SqlClient.SqlClient, sql))

...
const list = Command.make("list").pipe(
  Command.withHandler(Effect.fnUntraced(function*() {
    ...
    for (const p of providers) {
      ...
      yield* Console.log(`  ${colors.gray}client:${colors.reset}   ${colors.white}${p.config.clientName}${colors.reset}`)
      yield* Console.log(`  ${colors.gray}typeOnly:${colors.reset} ${p.config.isTypeOnly}`)
      if (p.config.patchList.length > 0) {
        yield* Console.log(`  ${colors.gray}patches:${colors.reset}  ${colors.yellow}${p.config.patchList.length} file(s)${colors.reset}`)
      }
      yield* Console.log("")
    }
  }))

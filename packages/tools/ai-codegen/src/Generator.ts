...
export interface CodeGenerator {
  readonly generate: (
    ...
    spec: unknown
  ) => Effect.Effect<string, GenerationError | PatchError>
}
...
> = Effect.gen(function*() {
  ...
  const generate = Effect.fn("generate")(function*(
    ...
    return yield* openApiGen
      .generate(patchedSpec as Parameters<typeof openApiGen.generate>[0], {
 ...
 * @category layers
 */
export const layerSchema: Layer.Layer<CodeGenerator> = layer.pipe(
 ...
 * @category layers
 */
export const layerTypeScript: Layer.Layer<CodeGenerator> = layer.pipe(

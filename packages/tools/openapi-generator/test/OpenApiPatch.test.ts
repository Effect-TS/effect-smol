  ...
  describe("parsePatchInput", () => {
    describe("inline JSON", () => {
      it.effect("parses valid inline JSON patch", () =>
        Effect.gen(function* () {
          const result = yield* OpenApiPatch.parsePatchInput('[{"op":"add","path":"/foo","value":"bar"}]')
      ...
      it.effect("fails on invalid JSON syntax", () =>
        Effect.gen(function* () {
          const exit = yield* Effect.exit(OpenApiPatch.parsePatchInput('[{"op":"add" "path":"/foo"}]'))
      ...
      it.effect("fails on unsupported operation", () =>
        Effect.gen(function* () {
          const exit = yield* Effect.exit(OpenApiPatch.parsePatchInput('[{"op":"copy","from":"/a","path":"/b"}]'))
          assert.isTrue(Exit.isFailure(exit))
        }).pipe(Effect.provide(testLayer))
      )
      ...
      it.effect("fails on missing path field", () =>
        Effect.gen(function* () {
          const exit = yield* Effect.exit(OpenApiPatch.parsePatchInput('[{"op":"add","value":"test"}]'))
          assert.isTrue(Exit.isFailure(exit))
        }).pipe(Effect.provide(testLayer))
      )
      ...
      it.effect("allows missing value for remove operation", () =>
        Effect.gen(function* () {
          const result = yield* OpenApiPatch.parsePatchInput('[{"op":"remove","path":"/foo"}]')
    ...
    describe("file paths", () => {
      it.effect("parses valid JSON file", () =>
        Effect.gen(function* () {
          ...
          const filePath = pathService.join(import.meta.dirname, "fixtures/patches/valid-add.json")
      ...
      it.effect("parses valid YAML file", () =>
        Effect.gen(function* () {
          const pathService = yield* Path.Path
          const filePath = pathService.join(import.meta.dirname, "fixtures/patches/valid-patch.yaml")
          const result = yield* OpenApiPatch.parsePatchInput(filePath)
      ...
      it.effect("parses multiple operations from JSON file", () =>
        Effect.gen(function* () {
          const pathService = yield* Path.Path
          const filePath = pathService.join(import.meta.dirname, "fixtures/patches/valid-multiple.json")
          const result = yield* OpenApiPatch.parsePatchInput(filePath)
          assert.strictEqual(result.length, 3)
        }).pipe(Effect.provide(testLayer))
      )
      ...
      it.effect("fails on file with unsupported operation", () =>
        Effect.gen(function* () {
          const pathService = yield* Path.Path
          const filePath = pathService.join(import.meta.dirname, "fixtures/patches/invalid-op.json")
      ...
      it.effect("fails on file with missing path", () =>
        Effect.gen(function* () {
          const pathService = yield* Path.Path
          const filePath = pathService.join(import.meta.dirname, "fixtures/patches/missing-path.json")
          const exit = yield* Effect.exit(OpenApiPatch.parsePatchInput(filePath))
          assert.isTrue(Exit.isFailure(exit))
        }).pipe(Effect.provide(testLayer))
      )
      ...
      it.effect("falls back to inline JSON when file does not exist", () =>
        Effect.gen(function* () {
          const result = yield* OpenApiPatch.parsePatchInput('[{"op":"add","path":"/x","value":1}]')
  ...
  describe("error messages", () => {
    it.effect("JsonPatchParseError has descriptive message", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(error.message, "Failed to parse patch from ./fix.json: Unexpected token")
      })
    )
    ...
    it.effect("JsonPatchValidationError has descriptive message", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(error.message, "Invalid JSON Patch from inline: Missing 'path' field")
      })
    )

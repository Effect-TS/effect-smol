 ...
 * @category models
 */
export type ParamKind = "option" | "positional"
...
export interface Param<out A, Kind extends ParamKind = ParamKind> extends Pipeable {
  ...
  readonly parse: ( // NOTE: method signatures in interfaces considered harmful!
    args: { readonly options: Record<string, ReadonlyArray<string>>; readonly operands: ReadonlyArray<string> }
  ) => Effect.Effect<
...
export interface Single<out A, Kind extends ParamKind = "option"> extends Param<A, Kind> {
...
> => {
  ...
  const rawValue = args.operands[0]
  ...
  return Effect.map(
    ...
    (value) => [args.operands.slice(1), value] as const
  )
}
...
const parseOption = <A>(
  ...
  FileSystem.FileSystem | Path.Path
> => {
  const providedValues = args.options[name]
  ...
  if (providedValues === undefined || providedValues.length === 0) {
    ...
    if (primitiveType._tag === "Boolean") {
      ...
      return Effect.succeed([args.operands, false as A] as const)
    } else {
      return Effect.fail(new CliError.MissingOption({ option: name }))
    }
  } else {
    ...
    return Effect.map(
      ...
      (value) => [args.operands, value] as const
    )
  }
}
...
> =>
  Effect.gen(function*() {
    let currentOperands = args.operands
...
> =>
  Effect.gen(function*() {
    ...
    const allValues = optionNames.flatMap((name) => args.options[name] || [])
    ...
    for (const value of allValues) {
      const [, parsedValue] = yield* param.parse({
        options: { [single.name]: [value] },
        operands: []
      })
      results.push(parsedValue)
    }

    return [args.operands, results] as const
  })
 ...
 * import * as Param from "@effect/cli/Param"
 *
 * const env = Param.keyValueMap("env", "option")
  ...
  return createWithCommonProto(CommonProto)<Optional<A, Kind>>({
    ...
    > {
      return Effect.catchTag(
        ...
        "MissingOption",
        () => Effect.succeed([args.operands, Option.none()] as const)
      )
    }
  })
}
 ...
 * import * as Param from "./Param"
 *
 * const config = Param.file("config", "option").pipe(
 *   Param.orElse(() => Param.string("config-url", "option"))
 * )
 * ```
 *
 * @since 4.0.0
 ...
 *
 * const configSource = Param.file("config", "option").pipe(
 *   Param.orElseEither(() => Param.string("config-url", "option"))
 * )

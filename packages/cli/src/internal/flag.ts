 ...
 * `effect/data/Option`.  Everything here is guaranteed to be of kind `"option"`.
 *
...
/**
 * Branded options type.  It behaves exactly like `Param.Param<A, "option">` (and therefore
 * like `Param.Single<A, "option">`) but carries an extra field keyed by a private symbol
 * so that external code cannot construct it directly.
 *
 * @since 4.0.0
 * @category models
 */
export interface Flag<A> extends Param.Param<A, "option"> {
...
function asFlag<A>(param: Param.Param<A, "option">): Flag<A> {
...
export const string = (name: string): Flag<string> => asFlag(Param.string(name, "option"))

export const boolean = (name: string): Flag<boolean> => asFlag(Param.boolean(name, "option"))

export const integer = (name: string): Flag<number> => asFlag(Param.integer(name, "option"))

export const float = (name: string): Flag<number> => asFlag(Param.float(name, "option"))

export const date = (name: string): Flag<Date> => asFlag(Param.date(name, "option"))
...
export const choiceWithValue = <const C extends ReadonlyArray<readonly [string, any]>>(
  name: string,
  choices: C
): Flag<C[number][1]> => asFlag(Param.choiceWithValue(name, choices, "option"))
...
export const choice = <const A extends ReadonlyArray<string>>(name: string, choices: A): Flag<A[number]> =>
  asFlag(Param.choice(name, choices, "option"))

export const path = (name: string, options?: {
  pathType?: "file" | "directory" | "either"
  mustExist?: boolean
  typeName?: string
}): Flag<string> => asFlag(Param.path(name, "option", options))

export const file = (name: string, options?: {
  mustExist?: boolean
}): Flag<string> => asFlag(Param.file(name, "option", options))

export const directory = (name: string, options?: {
  mustExist?: boolean
}): Flag<string> => asFlag(Param.directory(name, "option", options))

export const redacted = (name: string): Flag<Redacted.Redacted<string>> => asFlag(Param.redacted(name, "option"))
 ...
 */
export const fileContent = (name: string): Flag<string> => asFlag(Param.fileContent(name, "option"))
 ...
 * @category constructors
 */
export const fileText = (name: string): Flag<string> => asFlag(Param.fileText(name, "option"))
...
export const fileParse = <A>(
  ...
  format?: string
): Flag<A> => asFlag(Param.fileParse(name, schema, "option", format))
...
export const fileSchema = <A>(
  ...
  schema: Schema.Codec<A, string>,
  format?: string
): Flag<A> => asFlag(Param.fileSchema(name, schema, "option", format))
 ...
 * @category constructors
 */
export const keyValueMap = (name: string): Flag<Record<string, string>> => asFlag(Param.keyValueMap(name, "option"))
 ...
 * @category constructors
 */
export const none: Flag<never> = asFlag(Param.none("option"))

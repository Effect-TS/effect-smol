 ...
 * Everything here is guaranteed to be of kind `"positional"`.
 *
 * @since 4.0.0
 */
...
/**
 * Branded args type. It behaves exactly like `Param.Param<A, "positional">` but carries
 * an extra field keyed by a private symbol so that external code cannot construct it directly.
 *
 * @since 4.0.0
 * @category models
 */
export interface Argument<A> extends Param.Param<A, "positional"> {
...
function asArgument<A>(param: Param.Param<A, "positional">): Argument<A> {
 ...
 */
export const string = (name: string): Argument<string> => asArgument(Param.string(name, "positional"))
 ...
 * @category constructors
 */
export const integer = (name: string): Argument<number> => asArgument(Param.integer(name, "positional"))
...
export const file = (name: string, options?: { mustExist?: boolean }): Argument<string> =>
  asArgument(Param.file(name, "positional", options))
...
export const directory = (name: string, options?: { mustExist?: boolean }): Argument<string> =>
  asArgument(Param.directory(name, "positional", options))
 ...
 * @category constructors
 */
export const float = (name: string): Argument<number> => asArgument(Param.float(name, "positional"))
 ...
 * @category constructors
 */
export const date = (name: string): Argument<Date> => asArgument(Param.date(name, "positional"))
...
export const choice = <const A extends ReadonlyArray<string>>(
  name: string,
  choices: A
): Argument<A[number]> => asArgument(Param.choice(name, choices, "positional"))
...
export const path = (name: string, options?: {
  ...
  mustExist?: boolean
}): Argument<string> => asArgument(Param.path(name, "positional", options))
...
export const redacted = (name: string): Argument<Redacted.Redacted<string>> =>
  asArgument(Param.redacted(name, "positional"))
 ...
 * @category constructors
 */
export const fileContent = (name: string): Argument<string> => asArgument(Param.fileContent(name, "positional"))
 ...
 * @category constructors
 */
export const fileText = (name: string): Argument<string> => asArgument(Param.fileText(name, "positional"))
...
export const fileParse = <A>(
  ...
  format?: string
): Argument<A> => asArgument(Param.fileParse(name, schema, "positional", format))
...
export const fileSchema = <A>(
  ...
  schema: Schema.Codec<A, string>,
  format?: string
): Argument<A> => asArgument(Param.fileSchema(name, schema, "positional", format))
 ...
 * @category constructors
 */
export const none: Argument<never> = asArgument(Param.none("positional"))

...
import type { CommandConfig, InferConfig } from "./internal/config.ts"
...
import { extractBuiltInOptions, lex, parseArgs, ParsedCommandArgs } from "./internal/parseCommandArgs.ts"
 ...
 * @category models
 */
export interface Command<Name extends string, Input, E, R> extends Pipeable {
  readonly _tag: "Command"
  readonly name: Name
  readonly description: string
  readonly config: CommandConfig
  readonly subcommands: ReadonlyArray<Command<string, any, any, any>>
  readonly parsedConfig: ReturnType<typeof parseConfig>
 ...
 */
const parseParams = (
  parsedArgs: { readonly options: Record<string, ReadonlyArray<string>>; readonly operands: ReadonlyArray<string> },
  params: ReadonlyArray<Param<any>>
): Effect.Effect<ReadonlyArray<any>, CliError.CliError, Environment> => {
  return Effect.gen(function*() {
    const results: Array<any> = []
    let currentOperands = parsedArgs.operands

    for (const option of params) {
      const [remainingOperands, parsed] = yield* option.parse({
...
export const make: {
  ...
  name: string,
  config?: CommandConfig,
  handler?: (_: any) => Effect.Effect<void, any, any>
): Command<string, any, any, any> => {
...
const checkForDuplicateFlags = <Name extends string, Input>(
  parent: Command<Name, Input, any, any>,
  subcommands: ReadonlyArray<Command<string, any, any, any>>
): void => {
  ...
  extractNames(parent.parsedConfig.options)

  for (const subcommand of subcommands) {
    for (const option of subcommand.parsedConfig.options) {
      const singles = extractSingleParams(option)
      for (const single of singles) {
...
> => {
  ...
  const enhancedParse = (
    ...
    Effect.gen(function*() {
      ...
      const subResult = yield* sub.parse(input.subcommand.args)
...
// Helper types to extract error and context types from subcommands
type ExtractSubcommandErrors<T extends ReadonlyArray<Command<any, any, any, any>>> = T[number] extends
  Command<any, any, infer E, any> ? E : never
...
// Helper type to extract subcommand input types as a union
type ExtractSubcommandInputs<T extends ReadonlyArray<Command<any, any, any, any>>> = {
  [K in keyof T]: T[K] extends Command<infer N, infer I, any, any> ? { readonly name: N; readonly result: I }
    : never
}[number]
...
export const getHelpDoc = <Name extends string, Input>(
  command: Command<Name, Input, any, any>,
  commandPath?: ReadonlyArray<string>
): HelpDoc => {
  ...
  for (const arg of command.parsedConfig.args) {
  ...
  extractFlags(command.parsedConfig.options)
...
const getHelpForCommandPath = <Name extends string, Input, E, R>(
  ...
  let currentCommand: Command<string, any, any, any> = command as any
...
(input) =>
  Effect.gen(function*() {
    ...
    if (help) {
      const commandPath = [command.name, ...ParsedCommandArgs.getCommandPath(parsedArgs)]

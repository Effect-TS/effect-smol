  ...
  describe("polling", () => {
    ...
    it.effect("poll - should return Option.some when the deferred was completed", () =>
      Effect.gen(function* () {
        ...
        const result = yield* Deferred.poll(deferred).pipe(Effect.flatMap(UndefinedOr.getOrThrow))
    ...
    it.effect("poll - should return Option.some when the deferred was failed", () =>
      Effect.gen(function* () {
        ...
        const result = yield* Deferred.poll(deferred).pipe(Effect.flatMap(UndefinedOr.getOrThrow), Effect.exit)
        assert.deepStrictEqual(result, Exit.fail("boom"))
      })
    )

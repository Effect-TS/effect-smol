...
describe("Metric", () => {
  it.effect("should be referentially transparent", () =>
    Effect.gen(function* () {
      const id = nextId()
      const counter1 = Metric.counter(id).pipe(Metric.withAttributes(attributes), Metric.withConstantInput(1))
      const counter2 = Metric.counter(id).pipe(Metric.withAttributes(attributes), Metric.withConstantInput(1))
      const counter3 = Metric.counter(id).pipe(Metric.withAttributes({ z: "c" }), Metric.withConstantInput(1))
  ...
  it.effect("should dump the current state of all metrics", () =>
    Effect.gen(function* () {
      const counter1 = Metric.counter("counter").pipe(Metric.withAttributes(attributes), Metric.withConstantInput(1))
      const counter2 = Metric.counter("counter").pipe(Metric.withAttributes(attributes), Metric.withConstantInput(1))
      const counter3 = Metric.counter("counter").pipe(Metric.withAttributes({ z: "c" }), Metric.withConstantInput(1))
  ...
  describe("Counter", () => {
    it.effect("custom increment with value", () =>
      Effect.gen(function* () {
        const id = nextId()
        const counter = Metric.counter(id).pipe(Metric.withAttributes(attributes))
    ...
    it.effect("custom increment with constant", () =>
      Effect.gen(function* () {
        const id = nextId()
        const counter = Metric.counter(id).pipe(Metric.withAttributes(attributes), Metric.withConstantInput(1))
        yield* Effect.trackSuccesses(Effect.succeed(1), counter)
        yield* Effect.trackSuccesses(Effect.succeed(2), counter)
        const result = yield* Metric.value(counter)
    ...
    it.effect("custom increment with bigint value", () =>
      Effect.gen(function* () {
        const id = nextId()
        const counter = Metric.counter(id, { bigint: true }).pipe(Metric.withAttributes(attributes))
  ...
  describe("Gauge", () => {
    ...
    it.effect("custom set with constant", () =>
      Effect.gen(function* () {
        const id = nextId()
        const gauge = Metric.gauge(id).pipe(Metric.withAttributes(attributes), Metric.withConstantInput(1))
        yield* Effect.trackSuccesses(Effect.succeed(1), gauge)
        yield* Effect.trackSuccesses(Effect.succeed(2), gauge)
        const result = yield* Metric.value(gauge)
    ...
    it.effect("custom set with bigint value", () =>
      Effect.gen(function* () {
        const id = nextId()
        const gauge = Metric.gauge(id, { bigint: true }).pipe(Metric.withAttributes(attributes))
  ...
  describe("Frequency", () => {
    it.effect("custom occurence with value", () =>
      Effect.gen(function* () {
        const id = nextId()
        const frequency = Metric.frequency(id).pipe(Metric.withAttributes(attributes))
  ...
  describe("Histogram", () => {
    it.effect("custom observe with value", () =>
      Effect.gen(function* () {
        ...
        const histogram = Metric.histogram(id, { boundaries }).pipe(Metric.withAttributes(attributes))
  ...
  describe("trackDuration", () => {
    it.effect("tracks execution duration", () =>
      Effect.gen(function* () {
        ...
        const fiber = yield* Effect.sleep("1 hour").pipe(Effect.trackDuration(timer), Effect.forkChild)

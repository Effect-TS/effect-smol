...
describe("Effect", () => {
  ...
  describe("all", () => {
    it("tuple", () =>
      Effect.gen(function* () {
        const results = (yield* Effect.all([Effect.succeed(1), Effect.succeed(2), Effect.succeed(3)])) satisfies [
          number,
          number,
          number
        ]
  ...
  describe("acquireRelease", () => {
    it("releases on interrupt", () =>
      Effect.gen(function* () {
        let release = false
        const fiber = yield* Effect.acquireRelease(Effect.delay(Effect.succeed("foo"), 100), () =>
          Effect.sync(() => {
            release = true
          })
  ...
  describe("repeat", () => {
    it.effect("is interruptible", () =>
      Effect.gen(function* () {
        const fiber = yield* Effect.void.pipe(Effect.forever, Effect.timeoutOption(50), Effect.forkChild)
        yield* TestClock.adjust(50)
        const result = yield* Fiber.join(fiber)
    ...
    it.effect("repeat/until - repeats until a condition is true", () =>
      Effect.gen(function* () {
        ...
        const result = yield* decrement.pipe(Effect.tap(increment), Effect.repeat({ until: (n) => n === 0 }))
    ...
    it.effect("repeat/while - repeats while a condition is true", () =>
      Effect.gen(function* () {
        ...
        const decrement = Effect.sync(() => --input)
        const increment = Effect.sync(() => output++)
        const result = yield* decrement.pipe(Effect.tap(increment), Effect.repeat({ while: (n) => n > 0 }))
        assert.strictEqual(result, 0)
        assert.strictEqual(output, 10)
      })
    )
  ...
  describe("timeoutOption", () => {
    it.live("timeout a long computation", () =>
      Effect.gen(function* () {
        const result = yield* pipe(Effect.sleep(60_000), Effect.andThen(Effect.succeed(true)), Effect.timeoutOption(10))
        assert.deepStrictEqual(result, Option.none())
      })
    )
  ...
  describe("stack safety", () => {
    it.live("recursion", () => {
      const loop: Effect.Effect<void> = Effect.void.pipe(Effect.flatMap((_) => loop))
  ...
  describe("zip", () => {
    it.effect("concurrent: false", () => {
      ...
      const task1 = Effect.succeed("a").pipe(
        Effect.delay(50),
        Effect.tap(() => executionOrder.push("task1"))
      )
      const task2 = Effect.succeed(1).pipe(
        Effect.delay(1),
        Effect.tap(() => executionOrder.push("task2"))
      )
    ...
    describe("basic isolation", () => {
      ...
      it.effect("should isolate failures between parent and child transactions", () =>
        Effect.gen(function* () {
          ...
          const parentError = yield* Effect.atomic(
            Effect.gen(function* () {
              yield* TxRef.set(ref1, 10)
  ...
  describe("catchReason", () => {
    ...
    it.effect("catches matching reason - handler succeeds", () =>
      Effect.gen(function* () {
        const result = yield* Effect.fail(new AiError({ reason: new RateLimitError({ retryAfter: 60 }) })).pipe(
  ...
  describe("catchReasons", () => {
    ...
    it.effect("catches second reason type", () =>
      Effect.gen(function* () {
        const result = yield* Effect.fail(new AiError({ reason: new QuotaExceededError({ limit: 100 }) })).pipe(
          Effect.catchReasons("AiError", {
            RateLimitError: (r) => Effect.succeed(`rate: ${r.retryAfter}`),
            QuotaExceededError: (r) => Effect.succeed(`quota: ${r.limit}`)
          })
  ...
  describe("unwrapReason", () => {
    ...
    it.effect("extracts reason into error channel", () =>
      Effect.gen(function* () {
        const reason = new RateLimitError({ retryAfter: 60 })
        const exit = yield* Effect.fail(new AiError({ reason })).pipe(Effect.unwrapReason("AiError"), Effect.exit)

...
describe("Trie", () => {
  it("toString", () => {
    const trie = pipe(Trie.empty<number>(), Trie.insert("a", 0), Trie.insert("b", 1))
  ...
  it("keys alphabetical order", () => {
    ...
    const result = Array.from(Trie.keys(trie))
    deepStrictEqual(result, ["a", "ab", "abb", "abc", "adc", "b", "ba", "bac", "bca", "c", "ca", "cab", "cac", "dca"])
  })
  ...
  it("reduce", () => {
    ...
    strictEqual(trie.pipe(Trie.reduce("", (acc, _, key) => acc + key)), "sellssheshells")
  })
  ...
  it("Equal.symbol", () => {
    ...
    strictEqual(Equal.equals(Trie.make(["call", 0], ["me", 1]), Trie.make(["call", 0], ["me", 1])), true)
  })
})

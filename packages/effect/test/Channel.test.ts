  ...
  describe("constructors", () => {
    ...
    it.effect("fromIterator", () =>
      Effect.gen(function* () {
        const result = yield* Channel.fromIterator(() => ({
          n: 0,
          next(this: { n: number }) {
            return this.n === 5 ? { done: true, value: this.n } : { done: false, value: this.n++ }
          }
    ...
    it.effect("fromIteratorArray", () =>
      Effect.gen(function* () {
        ...
        const result = yield* Channel.runCollect(Channel.fromIteratorArray(() => fibonacci(), 3))
        assert.deepStrictEqual(result, [
          [0, 1, 1],
          [2, 3]
        ])
    ...
    it.effect("acquireRelease", () =>
      Effect.gen(function* () {
        ...
        yield* Channel.acquireRelease(Ref.set(acquired, true), () => Ref.set(released, true)).pipe(Channel.runDrain)
  ...
  describe("filtering", () => {
    it.effect("filterMap", () =>
      Effect.gen(function* () {
        ...
        const result = yield* Channel.fromArray([1, 2, 3, 4]).pipe(Channel.filterMap(filter), Channel.runCollect)
    ...
    it.effect("filterMapEffect", () =>
      Effect.gen(function* () {
        ...
        const result = yield* Channel.fromArray([1, 2, 3, 4]).pipe(Channel.filterMapEffect(filter), Channel.runCollect)
  ...
  describe("merging", () => {
    it.effect("merge - interrupts left side if halt strategy is set to 'right'", () =>
      Effect.gen(function* () {
        ...
        const right = Channel.fromQueue(leftQueue).pipe(Channel.ensuring(latch.open))
    ...
    it.effect("merge - interrupts right side if halt strategy is set to 'left'", () =>
      Effect.gen(function* () {
        ...
        const leftQueue = yield* Queue.make<number, Cause.Done>()
        const rightQueue = yield* Queue.make<number>()
        const left = Channel.fromQueue(leftQueue).pipe(Channel.ensuring(latch.open))
  ...
  describe("interruptWhen", () => {
    it.effect("interrupts the current element", () =>
      Effect.gen(function* () {
        ...
        yield* pipe(Deferred.await(started), Effect.andThen(Deferred.succeed(halt, void 0)))
    ...
    it.effect("interruptWhen - propagates errors", () =>
      Effect.gen(function* () {
        ...
        const channel = Channel.fromEffect(Effect.never).pipe(Channel.interruptWhen(Deferred.await(deferred)))

...
describe("DateTime", () => {
  describe("mutate", () => {
    ...
    it.effect("correctly preserves the time zone", () =>
      Effect.gen(function* () {
        yield* setTo2024NZ
        const now = yield* DateTime.nowInCurrentZone.pipe(DateTime.withCurrentZoneNamed("Pacific/Auckland"))
  ...
  describe("startOf", () => {
    ...
    it("month duplicated", () => {
      const mar = DateTime.makeUnsafe("2024-03-15T12:00:00.000Z")
      const end = DateTime.startOf(mar, "month").pipe(DateTime.startOf("month"))
      strictEqual(end.toJSON(), "2024-03-01T00:00:00.000Z")
    })

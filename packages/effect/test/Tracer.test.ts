...
describe("Tracer", () => {
  describe("Effect.withSpan", () => {
    ...
    it.effect("should set the parent span", () =>
      Effect.gen(function* () {
        const span = yield* Effect.currentSpan.pipe(Effect.withSpan("A"), Effect.withSpan("B"))
    ...
    it.effect("should override the parent span when root is set to true", () =>
      Effect.gen(function* () {
        const span = yield* Effect.currentSpan.pipe(Effect.withSpan("A", { root: true }), Effect.withSpan("B"))
  ...
  describe("Effect.annotateSpans", () => {
    it.effect("should allow adding span attributes as key/value pairs", () =>
      Effect.gen(function* () {
        const span = yield* Effect.currentSpan.pipe(Effect.withSpan("A"), Effect.annotateSpans("key", "value"))
  ...
  describe("Effect.withTracerEnabled", () => {
    it.effect("should allow enabling/disabling the tracer", () =>
      Effect.gen(function* () {
        const span = yield* Effect.currentSpan.pipe(Effect.withSpan("A"), Effect.withTracerEnabled(false))
        const spanB = yield* Effect.currentSpan.pipe(Effect.withSpan("B"), Effect.withTracerEnabled(true))
  ...
  describe("Effect.withTracerTiming", () => {
    ...
    it.effect("should not include timing information in spans when false", () =>
      Effect.gen(function* () {
        ...
        const span = yield* Effect.currentSpan.pipe(Effect.withSpan("A"), Effect.withTracerTiming(false))
  ...
  describe("Layer.parentSpan", () => {
    it.effect("should set the parent trace span for the layer constructor", () =>
      Effect.gen(function* () {
        ...
        Effect.provide(Layer.unwrap(Effect.map(Effect.makeSpanScoped("parent"), (span) => Layer.parentSpan(span))))
      )
    )
  })
  ...
  describe("Layer.withSpan", () => {
    it.effect("sets the trace span for the layer constructor", () =>
      Effect.gen(function* () {
        ...
        const span = yield* Effect.currentSpan.pipe(Effect.provide(layer), Effect.option)

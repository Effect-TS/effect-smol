...
describe("Pool", () => {
  ...
  it.live("blocks when item not available", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.scoped(Pool.get(pool)).pipe(Effect.forkChild({ startImmediately: true }))
  ...
  it.effect("compositional retry", () =>
    Effect.gen(function* () {
      ...
      const count = yield* Ref.make(0)
      const get = Effect.acquireRelease(Ref.updateAndGet(count, (n) => n + 1).pipe(Effect.flatMap(cond)), () =>
        Ref.update(count, (n) => n - 1)
      )
      const pool = yield* Pool.make({ acquire: get, size: 10 })
  ...
  it.effect("max pool size creation strategy", () =>
    Effect.gen(function* () {
      ...
      const acquire = Effect.acquireRelease(Effect.succeed("resource"), () => Ref.update(invalidated, (n) => n + 1))
      const pool = yield* Pool.makeWithTTL({
        acquire,
        min: 10,
        max: 15,
      ...
      const scope = yield* Scope.make()
      yield* Pool.get(pool).pipe(Effect.repeat({ times: 14 }), Scope.provide(scope))
  ...
  it.effect("shutdown robustness", () =>
    Effect.gen(function* () {
      ...
      yield* pipe(Effect.scoped(Pool.get(pool)), Effect.forkChild, Effect.repeat({ times: 99 }))
      yield* Scope.close(scope, Exit.succeed(void 0))
  ...
  it.effect("shutdown with pending takers", () =>
    Effect.gen(function* () {
      ...
      const scope = yield* Scope.make()
      const pool = yield* Scope.provide(Pool.make({ acquire: get, size: 10 }), scope)
      yield* pipe(Pool.get(pool), Scope.provide(scope), Effect.forkChild, Effect.repeat({ times: 99 }))
      yield* Scope.close(scope, Exit.succeed(void 0))
      const result = yield* Effect.repeat(Ref.get(count), { until: (n) => n === 0 })
      strictEqual(result, 0)
    })
  ...
  it.effect("finalizer is called for failed allocations", () =>
    Effect.gen(function* () {
      ...
      const pool = yield* Pool.make({ acquire: get, size: 10 }).pipe(Scope.provide(scope))

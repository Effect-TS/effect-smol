...
describe("TxPubSub", () => {
  describe("constructors", () => {
    it.effect("bounded creates pubsub with specified capacity", () =>
      Effect.gen(function*() {
        ...
        assert.strictEqual(yield* TxPubSub.isShutdown(hub), false)
      }))
  ...
  describe("publish and subscribe", () => {
    it.effect("publish with no subscribers is a no-op", () =>
      Effect.gen(function*() {
        const hub = yield* TxPubSub.unbounded<string>()
        const result = yield* TxPubSub.publish(hub, "hello")
      ...
      Effect.gen(function*() {
        ...
        yield* Effect.scoped(
          Effect.gen(function*() {
            ...
            const v1a = yield* TxQueue.take(sub1)
            const v1b = yield* TxQueue.take(sub1)
            const v2a = yield* TxQueue.take(sub2)
            const v2b = yield* TxQueue.take(sub2)
      ...
      Effect.gen(function*() {
        ...
        yield* Effect.scoped(
          Effect.gen(function*() {
            ...
            const fiber = yield* Effect.forkChild(TxPubSub.publish(hub, 3))
      ...
      Effect.gen(function*() {
        ...
        yield* Effect.scoped(
          Effect.gen(function*() {
            ...
            yield* TxPubSub.publish(hub, 1)
            yield* TxPubSub.publish(hub, 2)
            const accepted = yield* TxPubSub.publish(hub, 3) // should be dropped
      ...
      Effect.gen(function*() {
        ...
        yield* Effect.scoped(
          Effect.gen(function*() {
            ...
            for (let i = 0; i < 100; i++) {
              const accepted = yield* TxPubSub.publish(hub, i)
              assert.strictEqual(accepted, true)
            }

            for (let i = 0; i < 100; i++) {
              const v = yield* TxQueue.take(sub)
  ...
  describe("getters", () => {
    it.effect("size returns max subscriber queue size", () =>
      Effect.gen(function*() {
        ...
        assert.strictEqual(yield* TxPubSub.size(hub), 0)
        ...
        yield* Effect.scoped(
          Effect.gen(function*() {
            ...
            yield* TxPubSub.publish(hub, 1)
            yield* TxPubSub.publish(hub, 2)
            assert.strictEqual(yield* TxPubSub.size(hub), 2)

            yield* TxQueue.take(sub)
            assert.strictEqual(yield* TxPubSub.size(hub), 1)
          })
        )
      }))
    ...
    it.effect("isEmpty checks all subscriber queues are empty", () =>
      Effect.gen(function*() {
        const hub = yield* TxPubSub.unbounded<number>()
        assert.strictEqual(yield* TxPubSub.isEmpty(hub), true)
        ...
        yield* Effect.scoped(
          Effect.gen(function*() {
            ...
            yield* TxPubSub.publish(hub, 1)
            assert.strictEqual(yield* TxPubSub.isEmpty(hub), false)
    ...
    it.effect("isFull checks if any subscriber queue is at capacity", () =>
      Effect.gen(function*() {
        const hub = yield* TxPubSub.bounded<number>(2)
        assert.strictEqual(yield* TxPubSub.isFull(hub), false)
        ...
        yield* Effect.scoped(
          Effect.gen(function*() {
            ...
            yield* TxPubSub.publish(hub, 2)
            assert.strictEqual(yield* TxPubSub.isFull(hub), true)
          })
        )
      }))
  })
  ...
  describe("shutdown", () => {
    it.effect("shutdown prevents further publishes", () =>
      Effect.gen(function*() {
        ...
        assert.strictEqual(yield* TxPubSub.isShutdown(hub), true)
    ...
    it.effect("shutdown is idempotent", () =>
      Effect.gen(function*() {
        const hub = yield* TxPubSub.unbounded<number>()
        yield* TxPubSub.shutdown(hub)
        yield* TxPubSub.shutdown(hub) // second call should not throw
        assert.strictEqual(yield* TxPubSub.isShutdown(hub), true)
      }))
    ...
    it.effect("awaitShutdown returns immediately if already shutdown", () =>
      Effect.gen(function*() {
        const hub = yield* TxPubSub.unbounded<number>()
        yield* TxPubSub.shutdown(hub)
        yield* TxPubSub.awaitShutdown(hub) // should not block
      }))
  })
  ...
  describe("scope cleanup", () => {
    it.effect("closing scope removes subscriber", () =>
      Effect.gen(function*() {
        ...
        const result = yield* TxPubSub.publish(hub, 1)
        assert.strictEqual(result, true)
        assert.strictEqual(yield* TxPubSub.size(hub), 0)
      }))
      ...
      Effect.gen(function*() {
        ...
        yield* Effect.scoped(
          Effect.gen(function*() {
            ...
            yield* TxPubSub.publish(hub, 1)
            yield* TxPubSub.publish(hub, 2)
            results.push(yield* TxQueue.take(sub))
            results.push(yield* TxQueue.take(sub))
          })
        )
      ...
      Effect.gen(function*() {
        ...
        yield* Effect.scoped(
          Effect.gen(function*() {
            ...
            // Fork publishers
            const f1 = yield* Effect.forkChild(TxPubSub.publishAll(hub, [1, 2, 3]))
            const f2 = yield* Effect.forkChild(TxPubSub.publishAll(hub, [4, 5, 6]))
            ...
            for (let i = 0; i < 6; i++) {
              items1.push(yield* TxQueue.take(sub1))
              items2.push(yield* TxQueue.take(sub2))
            }

...
describe("FiberSet", () => {
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* Effect.onInterrupt(Effect.never, () => Ref.update(ref, (n) => n + 1)).pipe(
            FiberSet.run(set),
    ...
    Effect.gen(function* () {
      ...
      yield* pipe(
        Effect.gen(function* () {
          ...
          Array.range(1, 10).forEach(() => run(Effect.onInterrupt(Effect.never, () => Ref.update(ref, (n) => n + 1))))
          yield* Effect.yieldNow
        }),
        Effect.scoped
      )
  ...
  it.effect("propagateInterruption true", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.yieldNow
      yield* Fiber.interrupt(fiber)
      assertTrue(Exit.hasInterrupt(yield* FiberSet.join(set).pipe(Effect.exit)))
    })
  )

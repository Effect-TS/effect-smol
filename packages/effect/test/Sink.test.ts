...
describe("Sink", () => {
          ...
          Ref.make(Array.empty<number>()).pipe(
            Effect.flatMap((ref) =>
              stream.pipe(
                ...
                Stream.runCollect,
                Effect.flatMap((exit) => Ref.get(ref).pipe(Effect.map((result) => [exit, result])))
              )
            ),
            Effect.exit
          )
  ...
  describe("reduce", () => {
    it.effect("equivalence with Array.reduce", () =>
      Effect.gen(function* () {
        ...
        const result2 = yield* stream.pipe(Stream.runCollect, Effect.map(Array.reduce("", (s, n) => s + `${n}`)))
  ...
  describe("take", () => {
    it.effect("respects the given limit", () =>
      Effect.gen(function* () {
        const stream = Stream.make(1, 2, 3, 4).pipe(Stream.transduce(Sink.take<number>(3)))
    ...
    it.effect("produces empty trailing chunks", () =>
      Effect.gen(function* () {
        const stream = Stream.make(1, 2, 3, 4).pipe(Stream.transduce(Sink.take<number>(4)))
        const result = yield* Stream.runCollect(stream)
        deepStrictEqual(result, [[1, 2, 3, 4], []])
      })
    )
    ...
    it.effect("produces empty trailing chunks", () =>
      Effect.gen(function* () {
        const stream = Stream.empty.pipe(Stream.transduce(Sink.take<number>(3)))
        const result = yield* Stream.runCollect(stream)
  ...
  describe("flatMap", () => {
    ...
    it.effect("flatMap - with leftovers", () =>
      Effect.gen(function* () {
        const chunks = Array.make(Array.make(1, 2), Array.make(3, 4, 5), Array.empty<number>(), Array.make(7, 8, 9, 10))
        const sink = pipe(
          Sink.head<number>(),
          Sink.flatMap((head) =>
            pipe(

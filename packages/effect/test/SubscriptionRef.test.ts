...
describe("SubscriptionRef", () => {
  ...
  it.effect("concurrent subscribes and unsubscribes are handled correctly", () =>
    Effect.gen(function* () {
      const ref = yield* SubscriptionRef.make(0)
      const producer = yield* SubscriptionRef.update(ref, Number.increment).pipe(Effect.forever, Effect.forkScoped)
      const [result1, result2] = yield* Effect.all([makeConsumer(ref), makeConsumer(ref)], { concurrency: 2 })
...
const makeConsumer = Effect.fnUntraced(function* (ref: SubscriptionRef.SubscriptionRef<number>) {

...
describe("TxSubscriptionRef", () => {
  ...
  describe("subscriptions", () => {
    it.effect("changes includes current value as first element", () =>
      Effect.gen(function*() {
        const ref = yield* TxSubscriptionRef.make(100)

        yield* Effect.scoped(
          Effect.gen(function*() {
            const sub = yield* TxSubscriptionRef.changes(ref)
            const first = yield* TxQueue.take(sub)
      ...
      Effect.gen(function*() {
        ...
        yield* Effect.scoped(
          Effect.gen(function*() {
            ...
            yield* TxSubscriptionRef.set(ref, 3)

            assert.strictEqual(yield* TxQueue.take(sub), 1)
            assert.strictEqual(yield* TxQueue.take(sub), 2)
            assert.strictEqual(yield* TxQueue.take(sub), 3)
          })
        )
      }))
      ...
      Effect.gen(function*() {
        ...
        yield* Effect.scoped(
          Effect.gen(function*() {
            ...
            assert.strictEqual(yield* TxQueue.take(sub1), 0)
            assert.strictEqual(yield* TxQueue.take(sub2), 0)

            yield* TxSubscriptionRef.set(ref, 42)

            assert.strictEqual(yield* TxQueue.take(sub1), 42)
            assert.strictEqual(yield* TxQueue.take(sub2), 42)
          })
        )
      }))
    ...
    it.effect("scope close unsubscribes", () =>
      Effect.gen(function*() {
        ...
        yield* TxSubscriptionRef.set(ref, 1)
        assert.strictEqual(yield* TxSubscriptionRef.get(ref), 1)
      }))
  })

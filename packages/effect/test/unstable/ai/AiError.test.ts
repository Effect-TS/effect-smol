...
// =============================================================================
// AiErrorWithReason Tests
// =============================================================================

describe("AiErrorWithReason", () => {
  it("should delegate isRetryable to reason", () => {
    const retryableError = new AiError.AiErrorWithReason({
      module: "OpenAI",
      method: "completion",
    ...
    const nonRetryableError = new AiError.AiErrorWithReason({
      module: "OpenAI",
      method: "completion",
  ...
  it("should delegate retryAfter to reason", () => {
    const errorWithRetryAfter = new AiError.AiErrorWithReason({
      module: "OpenAI",
      method: "completion",
      reason: new AiError.RateLimitError({
        retryAfter: Duration.seconds(60)
    ...
    const errorWithoutRetryAfter = new AiError.AiErrorWithReason({
      module: "OpenAI",
      method: "completion",
      reason: new AiError.AuthenticationError({ kind: "InvalidKey" })
    })
  ...
  it("should format message with module, method, and reason", () => {
    const error = new AiError.AiErrorWithReason({
      module: "OpenAI",
      method: "completion",
  ...
  it("should have _tag set correctly", () => {
    ...
    assert.strictEqual(error._tag, "AiErrorWithReason")
  })
})
...
describe("AiError constructors", () => {
  describe("makeWithReason", () => {
    it("should create AiErrorWithReason", () => {
...
describe("AiError type guards", () => {
  describe("isAiError", () => {
    it("should return true for legacy AiError types", () => {
      ...
      assert.isTrue(AiError.isAiError(httpRequestError))

      const unknownError = new AiError.UnknownError({
        module: "Test",
        method: "test"
      })
      assert.isTrue(AiError.isAiError(unknownError))
    })
  ...
  describe("isAiErrorWithReason", () => {
    it("should return true for AiErrorWithReason", () => {
      const error = new AiError.AiErrorWithReason({
        module: "Test",
        method: "test",
        reason: new AiError.AiUnknownError({})
      })
      assert.isTrue(AiError.isAiErrorWithReason(error))
    })

    it("should return false for non-AiErrorWithReason values", () => {
      assert.isFalse(AiError.isAiErrorWithReason(new Error("regular error")))
      assert.isFalse(AiError.isAiErrorWithReason(null))
      assert.isFalse(AiError.isAiErrorWithReason(undefined))

      const legacyError = new AiError.UnknownError({
        module: "Test",
        method: "test"
      })
      assert.isFalse(AiError.isAiErrorWithReason(legacyError))
    })
  })
})
...
describe("AiError supporting schemas", () => {
...
// =============================================================================
// Schema Encoding/Decoding Tests
// =============================================================================

describe("AiError schema roundtrip", () => {
  ...
  it.effect("AiErrorWithReason roundtrip", () =>
    Effect.gen(function*() {
      const error = new AiError.AiErrorWithReason({
        module: "OpenAI",
        method: "completion",
      ...
      const encoded = yield* Schema.encodeEffect(AiError.AiErrorWithReason)(error)
      const decoded = yield* Schema.decodeEffect(AiError.AiErrorWithReason)(encoded)
      assert.strictEqual(decoded._tag, "AiErrorWithReason")

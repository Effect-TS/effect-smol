...
const FileSystemLayer = FileSystem.layerNoop({
  exists: (path) => (path.includes("/non/existent/file.txt") ? Effect.succeed(false) : Effect.succeed(true)),
  stat: (path) => {
...
describe("Command arguments", () => {
  ...
  it("should handle file mustExist validation", () =>
    Effect.gen(function* () {
      ...
      yield* Command.runWith(optionalFileCommand, { version: "1.0.0" })(["/non/existent/file.txt"])
  ...
  it("should handle variadic arguments", () =>
    Effect.gen(function* () {
      ...
      yield* Command.runWith(testCommand, { version: "1.0.0" })(["file1.txt", "file2.txt", "file3.txt"])
  ...
  it("should handle orElseResult combinator", () =>
    Effect.gen(function* () {
      ...
      const testCommand = Command.make(
        "test",
        {
          value: Argument.integer("value").pipe(Argument.orElseResult(() => Argument.string("value")))
        },
        (config) => Ref.set(resultRef, config)
      )
  ...
  it("should handle optional arguments - when provided", () =>
    Effect.gen(function* () {
      ...
      const testCommand = Command.make(
        "test",
        {
          label: Argument.optional(Argument.string("label").pipe(Argument.withDescription("Optional label name")))
        },
        (config) => Ref.set(resultRef, config)
      )
  ...
  it.effect("should show correct error message for invalid argument (not 'flag')", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.exit(intArg.parse({ flags: {}, arguments: ["not-a-number"] }))
      ...
      if (result._tag === "Failure") {
        ...
        assert.isTrue(errorMessage.includes("argument"), `Error message should say 'argument': ${errorMessage}`)
      }
    }).pipe(Effect.provide(TestLayer))
  )
})

...
const makeTestLayer = (testLogger: Logger.Logger<unknown, void>) =>
  Layer.mergeAll(FileSystemLayer, PathLayer, TerminalLayer, CliOutputLayer, SpawnerLayer, Logger.layer([testLogger]))
...
describe("LogLevel", () => {
  ...
  const filterLogs = (minLevel: string) => {
    ...
    return allLogs.filter((log) => severityOrder.indexOf(log.level) <= minSeverity).reverse()
  }
  ...
  // Test cases
  const testCases = ["all", "trace", "debug", "info", "warn", "warning", "error", "fatal", "none"]
  ...
  it.effect("should fail with InvalidValue for invalid log levels", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.flip(runCommand(["--log-level", "invalid"]).pipe(Effect.provide(TestLayer)))

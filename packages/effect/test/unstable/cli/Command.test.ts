  ...
  describe("withSubcommands", () => {
    ...
    it.effect("should provide parent context to subcommands", () =>
      Effect.gen(function* () {
        ...
        yield* runCommand(["--verbose", "--config", "prod.json", "child", "--action", "deploy"])
    ...
    it.effect("should accept parent flags before or after a subcommand (npm-style)", () =>
      Effect.gen(function* () {
        ...
        assert.deepStrictEqual(messages, ["install: global=true, pkg=cowsay", "install: global=true, pkg=cowsay"])
      }).pipe(Effect.provide(TestLayer))
    )
    ...
    it.effect("should allow direct accessing parent config in subcommands", () =>
      Effect.gen(function* () {
        ...
        assert.deepStrictEqual(messages, ["install: global=true, pkg=effect"])
      }).pipe(Effect.provide(TestLayer))
    )
    ...
    it.effect("should handle nested subcommands with context sharing", () =>
      Effect.gen(function* () {
        ...
        const serviceWithDeploy = service.pipe(Command.withSubcommands([deploy]))

        const appWithService = root.pipe(Command.withSubcommands([serviceWithDeploy]))

        const runCommand = Command.runWith(appWithService, { version: "1.0.0" })
        yield* runCommand(["--env", "production", "service", "--name", "api", "deploy", "--target-version", "1.0.0"])
    ...
    it.effect("should handle boolean flags before subcommands", () =>
      Effect.gen(function* () {
        ...
        assert.deepStrictEqual(messages, ["deploy: parent.verbose=true", "deploy: target-version=1.0.0"])
      }).pipe(Effect.provide(TestLayer))
    )
    ...
    it.effect("should support options before, after, or between operands (relaxed POSIX Syntax Guideline No. 9)", () =>
      Effect.gen(function* () {
        ...
        yield* Cli.run(["copy", "--recursive", "--force", "src.txt", "dest.txt"])

        // Test 2: Modern style - options after operands
        yield* Cli.run(["copy", "src.txt", "dest.txt", "--recursive", "--force"])

        // Test 3: Mixed style - some options before, some after
        yield* Cli.run(["copy", "--recursive", "src.txt", "dest.txt", "--force"])

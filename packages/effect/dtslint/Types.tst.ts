...
describe("Types", () => {
  ...
  it("TupleOfAtLeast", () => {
    expect<Types.TupleOfAtLeast<3, number>>().type.toBe<[number, number, number, ...Array<number>]>()
  })

  it("UnionToIntersection", () => {
    expect<Types.UnionToIntersection<{ a: string } | { b: number }>>().type.toBe<{ a: string; b: number }>()
  })

  it("Tags", () => {
    expect<Types.Tags<string | { _tag: "a" } | { _tag: "b" }> & unknown>().type.toBe<"a" | "b">()
  })

  it("ExcludeTag", () => {
    expect<Types.ExcludeTag<string | { _tag: "a" } | { _tag: "b" }, "a"> & unknown>().type.toBe<
      string | { _tag: "b" }
    >()
  })

  it("ExtractTag", () => {
    expect<Types.ExtractTag<string | { _tag: "a"; a: number } | { _tag: "b"; b: number }, "b"> & unknown>().type.toBe<{
      _tag: "b"
      b: number
    }>()
  })
  ...
  it("Simplify", () => {
    expect<Types.Simplify<object & { a: number } & { b: number }>>().type.toBe<{ a: number; b: number }>()
  })
  ...
  describe("MergeRight", () => {
    it("mutable overwrites mutable", () => {
      expect<Types.MergeRight<{ a: number; b: number }, { a: string; c: boolean }>>().type.toBe<{
        a: string
        c: boolean
        b: number
      }>()
    ...
    it("mutable overwrites readonly", () => {
      expect<Types.MergeRight<{ readonly a: number; b: number }, { a: string; c: boolean }>>().type.toBe<{
        a: string
        c: boolean
        b: number
      }>()
    ...
    it("readonly overwrites mutable", () => {
      expect<Types.MergeRight<{ a: number; b: number }, { readonly a: string; c: boolean }>>().type.toBe<{
        readonly a: string
        c: boolean
        b: number
      }>()
    ...
    it("optionality of non involved keys must be preserved", () => {
      expect<Types.MergeRight<{ readonly a?: number; b: number }, { readonly c?: string }>>().type.toBe<{
        readonly c?: string
        readonly a?: number
        b: number
      }>()
  ...
  describe("Mutable", () => {
    it("should convert a readonly object to mutable", () => {
      expect<Types.Mutable<{ readonly a: string; readonly b: number }>>().type.toBe<{ a: string; b: number }>()
    })
    ...
    it("should convert a readonly tuple to a mutable tuple", () => {
      expect<Types.Mutable<readonly [string, number]>>().type.toBe<[string, number]>()
    })

    it("should convert a readonly record to a mutable record", () => {
      expect<Types.Mutable<{ readonly [x: string]: number }>>().type.toBe<{ [x: string]: number }>()
    })
  })
  ...
  describe("DeepMutable", () => {
    ...
    describe("Index Signature", () => {
      it("should convert an readonly Index Signature to a mutable Index Signature", () => {
        expect<Types.DeepMutable<{ readonly [x: string]: number }>>().type.toBe<{ [x: string]: number }>()
      })

      it("should leave an Index Signature unchanged", () => {
        expect<Types.DeepMutable<{ [_: string]: number }>>().type.toBe<{ [x: string]: number }>()
      })
    })
    ...
    describe("Struct", () => {
      ...
      it("should deeply mutate nested structs", () => {
        expect<Types.DeepMutable<ReadonlyArray<TaggedValues<TaggedValues<TaggedValues<boolean>>>>>>().type.toBe<
          Array<{
            _tag: string
            value: Array<{
              _tag: string
    ...
    describe("Array", () => {
      ...
      it("should convert a readonly array to a mutable array", () => {
        expect<Types.DeepMutable<ReadonlyArray<string>>>().type.toBe<Array<string>>()
      })
    ...
    describe("Tuple", () => {
      it("should convert a readonly tuple", () => {
        expect<Types.DeepMutable<readonly [string, number, boolean]>>().type.toBe<[string, number, boolean]>()
      })

      it("should leave a mutable tuple unchanged", () => {
        expect<Types.DeepMutable<[string, number, boolean]>>().type.toBe<[string, number, boolean]>()
      })
    })

    describe("ReadonlySet", () => {
      it("should convert a ReadonlySet to a mutable Set", () => {
        expect<Types.DeepMutable<ReadonlySet<{ readonly value: TaggedValues<number> }>>>().type.toBe<
          Set<{ value: { _tag: string; value: Array<number> } }>
        >()
      })

      it("should leave a mutable Set unchanged", () => {
        expect<Types.DeepMutable<Set<{ readonly value: TaggedValues<number> }>>>().type.toBe<
          Set<{ value: { _tag: string; value: Array<number> } }>
        >()
      })
    })
    ...
    describe("ReadonlyMap", () => {
      it("should convert a ReadonlyMap to a mutable Map", () => {
        expect<Types.DeepMutable<ReadonlyMap<TaggedValues<string>, ReadonlySet<TaggedValues<number>>>>>().type.toBe<
          Map<{ _tag: string; value: Array<string> }, Set<{ _tag: string; value: Array<number> }>>
        >()
      })

      it("should leave a mutable Map unchanged", () => {
        expect<Types.DeepMutable<Map<TaggedValues<string>, ReadonlySet<TaggedValues<number>>>>>().type.toBe<
          Map<{ _tag: string; value: Array<string> }, Set<{ _tag: string; value: Array<number> }>>
        >()
      })
    })

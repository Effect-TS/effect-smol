...
const TypeMatcherProto: Omit<TypeMatcher<any, any, any, any>, "cases"> = {
  ...
  _tag: "TypeMatcher",
  add<I, R, RA, A>(this: TypeMatcher<any, any, any, any>, _case: Case): TypeMatcher<I, R, RA, A> {
...
function makeTypeMatcher<I, R, RA, A>(cases: ReadonlyArray<Case>): TypeMatcher<I, R, RA, A> {
  const matcher = Object.create(TypeMatcherProto)
  matcher.cases = cases
  return matcher
}

const ValueMatcherProto: Omit<ValueMatcher<any, any, any, any, any>, "provided" | "value"> = {
  [TypeId]: {
    _input: identity,
    _filters: identity,
    _result: identity,
  ...
  _tag: "ValueMatcher",
  add<I, R, RA, A, Pr>(this: ValueMatcher<any, any, any, any, any>, _case: Case): ValueMatcher<I, R, RA, A, Pr> {
...
function makeValueMatcher<I, R, RA, A, Pr>(provided: Pr, value: Result.Result<Pr, RA>): ValueMatcher<I, R, RA, A, Pr> {
...
const makeWhen = (guard: (u: unknown) => boolean, evaluate: (input: unknown) => any): When => ({
  _tag: "When",
  guard,
  evaluate
})

const makeNot = (guard: (u: unknown) => boolean, evaluate: (input: unknown) => any): Not => ({
  _tag: "Not",
  guard,
  evaluate
})
...
const makePredicate = (pattern: unknown): Predicate.Predicate<unknown> => {
  if (typeof pattern === "function") {
    ...
    const keysAndPredicates = Object.entries(pattern).map(([k, p]) => [k, makePredicate(p)] as const)
...
const makeOrPredicate = (patterns: ReadonlyArray<unknown>): Predicate.Predicate<unknown> => {
...
const makeAndPredicate = (patterns: ReadonlyArray<unknown>): Predicate.Predicate<unknown> => {
  const predicates = patterns.map(makePredicate)
  const len = predicates.length
...
/** @internal */
export const type = <I>(): Matcher<I, Types.Without<never>, I, never, never> => makeTypeMatcher([])
...
export const whenAnd =
  ...
  <I, F, A, Pr>(
    ...
    Types.ApplyFilters<I, Types.AddWithout<F, Types.PForExclude<Types.ArrayToIntersection<P>>>>,
    A | ReturnType<Fn>,
    Pr
  > => {
    const onMatch = args[args.length - 1] as any
...
export const discriminator =
  ...
  <R, P extends Types.Tags<D, R> & string, Ret, Fn extends (_: Extract<R, Record<D, P>>) => Ret>(
    ...
    const pred = values.length === 1 ? (_: any) => _[field] === values[0] : (_: any) => values.includes(_[field])
...
export const discriminatorStartsWith =
  <D extends string>(field: D) =>
  <R, P extends string, Ret, Fn extends (_: Extract<R, Record<D, `${P}${string}`>>) => Ret>(pattern: P, f: Fn) => {
...
/** @internal */
export const is: <Literals extends ReadonlyArray<string | number | boolean | null | bigint>>(
  ...literals: Literals
...
export const result: <I, F, R, A, Pr, Ret>(
  self: Matcher<I, F, R, A, Pr, Ret>
) => [Pr] extends [never] ? (input: I) => Result.Result<Unify<A>, R> : Result.Result<Unify<A>, R> = (<I, R, RA, A>(
...
export const option: <I, F, R, A, Pr, Ret>(
  self: Matcher<I, F, R, A, Pr, Ret>
) => [Pr] extends [never] ? (input: I) => Option.Option<Unify<A>> : Option.Option<Unify<A>> = (<I, A>(
...
) => [Pr] extends [never] ? (u: I) => Unify<A> : Unify<A> = (<I, F, A>(self: Matcher<I, F, never, A, I>) => {

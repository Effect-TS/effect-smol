...
> {
   ...
   */
  annotate<I, S>(key: ServiceMap.Service<I, S>, value: S): Workflow<Name, Payload, Success, Error>
 ...
 * @since 4.0.0
 */
export interface CompleteSchema<Success extends Schema.Top, Error extends Schema.Top> extends Schema.declareConstructor<
 ...
 * @category Result
 */
export const Result = <Success extends Schema.Top, Error extends Schema.Top>(options: {
  readonly success: Success
  readonly error: Error
 ...
 * @category Scope
 */
export const scope: Effect.Effect<Scope.Scope, never, WorkflowInstance> = Effect.map(
...
) => Effect.Effect<void, never, WorkflowInstance | R> = Effect.fnUntraced(function* <R>(
  f: (exit: Exit.Exit<unknown, unknown>) => Effect.Effect<void, never, R>
) {
  ...
  <A, E, R, R2>(
    ...
    Effect.uninterruptibleMask((restore) =>
      Effect.tap(restore(effect), (value) =>
        addFinalizer((exit) => (Exit.isSuccess(exit) ? Effect.void : compensation(value, exit.cause)))
      )
    )
)

...
export class WorkflowEngine extends ServiceMap.Service<
  WorkflowEngine,
  {
     ...
     */
    readonly activityExecute: <Success extends Schema.Top, Error extends Schema.Top, R>(
     ...
     */
    readonly deferredResult: <Success extends Schema.Top, Error extends Schema.Top>(
     ...
     * workflows.
     */
    readonly deferredDone: <Success extends Schema.Top, Error extends Schema.Top>(
      deferred: DurableDeferred.DurableDeferred<Success, Error>,
      options: {
...
export const makeUnsafe = (options: Encoded): WorkflowEngine["Service"] =>
  WorkflowEngine.of({
    ...
    activityExecute: Effect.fnUntraced(function* <Success extends Schema.Top, Error extends Schema.Top, R>(
      activity: Activity.Activity<Success, Error, R>,
      attempt: number
    ) {
    ...
    deferredResult: Effect.fnUntraced(
      function* <Success extends Schema.Top, Error extends Schema.Top>(
        ...
        return yield* Effect.orDie(Schema.decodeEffect(deferred.exitSchema)(toJsonExit(exit))) as Effect.Effect<
    ...
    deferredDone: Effect.fnUntraced(
      function* <Success extends Schema.Top, Error extends Schema.Top>(
        ...
        return yield* options.deferredDone({
          ...
          exit: yield* Schema.encodeEffect(deferred.exitSchema)(opts.exit) as Effect.Effect<Exit.Exit<unknown, unknown>>
        })
      },
      Effect.withSpan(
...
const defaultRetrySchedule = Schedule.exponential(200, 1.5).pipe(Schedule.either(Schedule.spaced(30000)))

 ...
 * @since 4.0.0
 */
export type ResolvedMigration = readonly [id: number, name: string, load: Effect.Effect<any, any, Client.SqlClient>]
...
export const make =
  ...
  name VARCHAR(255) NOT NULL
      ...
      const insertMigrations = (rows: ReadonlyArray<[id: number, name: string]>) =>
        sql`INSERT INTO ${sql(table)} ${sql.insert(rows.map(([migration_id, name]) => ({ migration_id, name })))}`
          .withoutTransform

      const latestMigration = Effect.map(
        sql<{ migration_id: number; name: string; created_at: Date }>`SELECT migration_id, name, created_at FROM ${sql(
          table
      ...
      const runMigration = (id: number, name: string, effect: Effect.Effect<unknown, unknown, Client.SqlClient>) =>
      ...
      const completed = yield* pipe(
        ...
        Effect.catchTag("MigrationError", (_) =>
          _.reason === "locked" ? Effect.as(Effect.logDebug(_.message), []) : Effect.fail(_)
        )
      )
 ...
 * @category loaders
 */
export const fromGlob = (migrations: Record<string, () => Promise<any>>): Loader =>
  pipe(
    Object.keys(migrations),
    Arr.filterMap((_) => Option.fromNullishOr(_.match(/^(?:.*\/)?(\d+)_([^.]+)\.(js|ts)$/))),
    Arr.map(([key, id, name]): ResolvedMigration => [Number(id), name, Effect.promise(() => migrations[key]())]),
...
export const fromBabelGlob = (migrations: Record<string, any>): Loader =>
  pipe(
    ...
    Arr.map(([key, id, name]): ResolvedMigration => [Number(id), name, Effect.succeed(migrations[key])]),
    Arr.sort(migrationOrder),
    Effect.succeed
  )

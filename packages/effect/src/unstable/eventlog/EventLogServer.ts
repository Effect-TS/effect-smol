...
> = Effect.gen(function*() {
  ...
  function* handler(socket: Socket.Socket) {
    ...
    yield* Effect.forkScoped(Effect.orDie(write(new Hello({ remoteId }))))
...
export class PersistedEntry extends Schema.Class<PersistedEntry>(
  "effect/unstable/EventLogServer/PersistedEntry"
)({
  entryId: EntryId,
 ...
 * @category storage
 */
export const Storage: ServiceMap.Service<Storage, Storage> = ServiceMap.Service(
  "effect/unstable/EventLogServer/Storage"
)
 ...
 * @category storage
 */
export const makeStorageMemory: Effect.Effect<Storage, never, Scope.Scope> = Effect.scoped(
  Effect.gen(function*() {
    ...
    return Storage.of({
      ...
      changes: (publicKey, startSequence) =>
        Effect.gen(function*() {
          const queue = yield* Queue.make<EncryptedRemoteEntry, Queue.Done>()
          ...
          yield* PubSub.takeBetween(subscription, 1, Number.MAX_SAFE_INTEGER).pipe(
            Effect.tap((chunk) => Queue.offerAll(queue, chunk)),
            Effect.forever,
            Effect.forkScoped,

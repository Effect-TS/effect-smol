 ...
 * @category Models
 */
export interface AtomRpcClient<Self, Id extends string, Rpcs extends Rpc.Any, E> extends ServiceMap.Service<
  Self,
  RpcClient.RpcClient.Flat<Rpcs, RpcClientError>
> {
  new (_: never): ServiceMap.ServiceClass.Shape<Id, RpcClient.RpcClient.Flat<Rpcs, RpcClientError>>
  ...
  >
    ? [_Success] extends [RpcSchema.Stream<infer _A, infer _E>]
      ? Atom.Writable<Atom.PullResult<_A["Type"], _E["Type"] | _Error["Type"] | E | _Middleware["error"]["Type"]>, void>
      : Atom.Atom<AsyncResult.AsyncResult<_Success["Type"], _Error["Type"] | E | _Middleware["error"]["Type"]>>
    : never
}
  ...
  >(
    ...
    const queryFamily = Atom.family(({ headers, payload, reactivityKeys, tag, timeToLive }: QueryKey) => {
      ...
      let atom = RpcSchema.isStreamSchema(rpc.successSchema)
        ? self.runtime.pull(
            Stream.unwrap(self.use((client) => Effect.succeed(client(tag, payload, { headers } as any) as any)))
          )
        : self.runtime.atom(self.use((client) => client(tag, payload, { headers } as any)) as any)
      if (timeToLive) {
    ...
    self.query = <Tag extends Rpc.Tag<Rpcs>>(
      ...
      queryFamily({
        ...
        timeToLive: options?.timeToLive ? Duration.fromDurationInputUnsafe(options.timeToLive) : undefined
      }) as any

 ...
 * @category Models
 */
export interface AtomHttpApiClient<Self, Id extends string, Groups extends HttpApiGroup.Any> extends ServiceMap.Service<
  Self,
  Simplify<HttpApiClient.Client<Groups, never, never>>
> {
...
export const Service =
  ...
  <const Id extends string, ApiId extends string, Groups extends HttpApiGroup.Any>(
    ...
    self.layer = Layer.effect(self, HttpApiClient.make(options.api, options)).pipe(
      Layer.provide(options.httpClient)
    ) as Layer.Layer<Self>
    ...
    self.query = ((
      ...
      queryFamily({
        ...
        timeToLive: request.timeToLive ? Duration.fromDurationInputUnsafe(request.timeToLive) : undefined
      })) as any

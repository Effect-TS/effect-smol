...
describe("Stream", () => {
  ...
  it.effect("fromDuplex should work with node:zlib", () =>
    Effect.gen(function* () {
      ...
      const result = yield* stream.pipe(Stream.pipeThroughChannelOrFail(channel), Stream.decodeText(), Stream.mkString)
  ...
  it.effect("toReadable roundtrip", () =>
    Effect.gen(function* () {
      ...
      const items = yield* outStream.pipe(Stream.decodeText(), Stream.runCollect)

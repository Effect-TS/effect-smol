  ...
  it.layer(NodeServices)((it) => {
    describe("spawn", () => {
      ...
      describe("combined output (all)", () => {
        ...
        it.effect("should allow reading .all independently", () =>
          Effect.gen(function* () {
            const handle = yield* ChildProcess.make("sh", ["-c", "echo out; sleep 0.01; echo err >&2"])
            const all = yield* decodeByteStream(handle.all)
            const exitCode = yield* handle.exitCode
    ...
    describe("error handling", () => {
      ...
      it.effect("should fail for invalid command", () =>
        Effect.gen(function* () {
          const exit = yield* Effect.exit(ChildProcess.make("nonexistent-command-12345").asEffect())
    ...
    describe("additionalFds", () => {
      ...
      it.effect("should handle multiple additional fds", () =>
        Effect.gen(function* () {
          ...
          const handle = yield* ChildProcess.make("sh", ["-c", "echo 'output on fd3' >&3; echo 'output on fd4' >&4"], {
            additionalFds: {
              fd3: { type: "output" },
              fd4: { type: "output" }
            }
      ...
      it.effect("should handle fd gaps (e.g., fd3 and fd5 without fd4)", () =>
        Effect.gen(function* () {
          ...
          const handle = yield* ChildProcess.make("sh", ["-c", "echo 'on fd3' >&3; echo 'on fd5' >&5"], {
            additionalFds: {
              fd3: { type: "output" },
              fd5: { type: "output" }
            }
      ...
      it.effect("should handle bidirectional communication via separate fds", () =>
        Effect.gen(function* () {
          ...
          const handle = yield* ChildProcess.make("sh", ["-c", "cat <&3 | tr a-z A-Z >&4"], {
            additionalFds: {
              fd3: { type: "input", stream: inputStream },
              fd4: { type: "output" }
            }

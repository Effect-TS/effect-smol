...
describe("HttpApi", () => {
  ...
  describe("middleware", () => {
    ...
    it.effect("client middleware mutates headers and propagates metadata", () => {
      class Metadata extends HttpApiMiddleware.Service<Metadata>()("Http/Metadata", {
        ...
        (handlers) =>
          handlers.handle("a", (ctx) =>
            Effect.succeed({
              ...
              endpoint: ctx.request.headers["x-endpoint"] ?? "",
              group: ctx.request.headers["x-group"] ?? ""
            }))
      )
      const MetadataLive = Layer.succeed(Metadata, Metadata.of((httpEffect) => httpEffect))
      const MetadataClient = HttpApiMiddleware.layerClient(Metadata, ({ endpoint, group, next, request }) =>
        next(
          HttpClientRequest.setHeader(
            HttpClientRequest.setHeader(
              HttpClientRequest.setHeader(request, "x-client", "set"),
              "x-endpoint",
              endpoint.name
            ),
            "x-group",
      ...
      const ApiLive = HttpRouter.serve(
        HttpApiBuilder.layer(Api).pipe(Layer.provide(GroupLive), Layer.provide(MetadataLive)),
        { disableListenLog: true, disableLogger: true }
      ).pipe(Layer.provideMerge(NodeHttpServer.layerTest))
      ...
      return Effect.gen(function*() {
        ...
        const expectRequiredClient = (
          _: Effect.Effect<
            unknown,
            unknown,
            HttpApiMiddleware.ForClient<ServiceMap.Service.Identifier<typeof Metadata>>
          >
        ) => _
        expectRequiredClient(client.group.a())

        yield* assertClientJson(
          client.group.a().pipe(Effect.provide(MetadataClient)),
          {
            client: "set",
            endpoint: "a",
            group: "group"
    ...
    it.effect("client middleware runs in LIFO order", () => {
      class First extends HttpApiMiddleware.Service<First>()("Http/First") {}
      class Second extends HttpApiMiddleware.Service<Second>()("Http/Second") {}
      ...
      const GroupLive = HttpApiBuilder.group(
        Api,
        "group",
        (handlers) => handlers.handle("a", (ctx) => Effect.succeed(ctx.request.headers["x-order"] ?? ""))
      )
      const FirstLive = Layer.succeed(First, First.of((httpEffect) => httpEffect))
      const SecondLive = Layer.succeed(Second, Second.of((httpEffect) => httpEffect))
      const FirstClient = HttpApiMiddleware.layerClient(First, ({ next, request }) => {
        const order = Headers.get(request.headers, "x-order") ?? ""
        return next(HttpClientRequest.setHeader(request, "x-order", `${order}1`))
      })
      const SecondClient = HttpApiMiddleware.layerClient(Second, ({ next, request }) => {
        const order = Headers.get(request.headers, "x-order") ?? ""
        return next(HttpClientRequest.setHeader(request, "x-order", `${order}2`))
      })

      const ApiLive = HttpRouter.serve(
        HttpApiBuilder.layer(Api).pipe(Layer.provide(GroupLive), Layer.provide(FirstLive), Layer.provide(SecondLive)),
        { disableListenLog: true, disableLogger: true }
      ).pipe(Layer.provideMerge(NodeHttpServer.layerTest))
      ...
      return Effect.gen(function*() {
        ...
        yield* assertClientText(
          client.group.a().pipe(Effect.provide(FirstClient), Effect.provide(SecondClient)),
          "21"
        )
      }).pipe(Effect.provide(ApiLive))
    })

    it.effect("required and optional client middleware behavior", () => {
      class Required extends HttpApiMiddleware.Service<Required>()("Http/Required", {
        requiredForClient: true
      }) {}
      class Optional extends HttpApiMiddleware.Service<Optional>()("Http/Optional") {}

      const Api = HttpApi.make("api").add(
        HttpApiGroup.make("group").add(
          HttpApiEndpoint.get("required", "/required", {
            success: Schema.String
          }).middleware(Required),
          HttpApiEndpoint.get("optional", "/optional", {
            success: Schema.String
          }).middleware(Optional)
        )
      )
      const GroupLive = HttpApiBuilder.group(
        ...
        (handlers) =>
          handlers
            .handle("required", (ctx) => Effect.succeed(`required:${ctx.request.headers["x-required"] ?? "missing"}`))
            .handle("optional", (ctx) => Effect.succeed(`optional:${ctx.request.headers["x-optional"] ?? "missing"}`))
      )
      const RequiredLive = Layer.succeed(Required, Required.of((httpEffect) => httpEffect))
      const OptionalLive = Layer.succeed(Optional, Optional.of((httpEffect) => httpEffect))
      const RequiredClient = HttpApiMiddleware.layerClient(
        Required,
        ({ next, request }) => next(HttpClientRequest.setHeader(request, "x-required", "provided"))
      )
      const OptionalClient = HttpApiMiddleware.layerClient(
        Optional,
        ({ next, request }) => next(HttpClientRequest.setHeader(request, "x-optional", "provided"))
      )

      const ApiLive = HttpRouter.serve(
        HttpApiBuilder.layer(Api).pipe(
          Layer.provide(GroupLive),
          Layer.provide(RequiredLive),
          Layer.provide(OptionalLive)
        ),
        { disableListenLog: true, disableLogger: true }
      ).pipe(Layer.provideMerge(NodeHttpServer.layerTest))
      ...
      return Effect.gen(function*() {
        ...
        const expectRequiredClient = (
          _: Effect.Effect<
            unknown,
            unknown,
            HttpApiMiddleware.ForClient<ServiceMap.Service.Identifier<typeof Required>>
          >
        ) => _
        const expectNoClientRequirement = (_: Effect.Effect<unknown, unknown, never>) => _

        expectRequiredClient(client.group.required())
        expectNoClientRequirement(client.group.optional())

        yield* assertClientText(client.group.optional(), "optional:missing")
        yield* assertClientText(client.group.required().pipe(Effect.provide(RequiredClient)), "required:provided")
        yield* assertClientText(client.group.optional().pipe(Effect.provide(OptionalClient)), "optional:provided")
      }).pipe(Effect.provide(ApiLive))
    })

    it.effect("typed clientError from middleware", () => {
      class ClientMiddlewareError extends Schema.ErrorClass<ClientMiddlewareError>("ClientMiddlewareError")({
        _tag: Schema.tag("ClientMiddlewareError")
      }) {}

      class FailClient extends HttpApiMiddleware.Service<FailClient, {
        clientError: ClientMiddlewareError
      }>()("Http/FailClient", {
        requiredForClient: true
      }) {}
      ...
      const FailClientLive = Layer.succeed(FailClient, FailClient.of((httpEffect) => httpEffect))
      const FailClientMiddleware = HttpApiMiddleware.layerClient(
        FailClient,
        () => Effect.fail(new ClientMiddlewareError({}))
      )

      const ApiLive = HttpRouter.serve(
        HttpApiBuilder.layer(Api).pipe(Layer.provide(GroupLive), Layer.provide(FailClientLive)),
        { disableListenLog: true, disableLogger: true }
      ).pipe(Layer.provideMerge(NodeHttpServer.layerTest))

      return Effect.gen(function*() {
        const client = yield* HttpApiClient.make(Api)
        const value = yield* client.group.a().pipe(
          Effect.catchTag("ClientMiddlewareError", () => Effect.succeed("caught")),
          Effect.provide(FailClientMiddleware)
        )
        assert.strictEqual(value, "caught")
      }).pipe(Effect.provide(ApiLive))
    })

    it.effect("effectful layerClient", () => {
      class Token extends ServiceMap.Service<Token, string>()("Token") {}
      class Effectful extends HttpApiMiddleware.Service<Effectful>()("Http/Effectful", {
        requiredForClient: true
      }) {}
      ...
      const Api = HttpApi.make("api").add(
        HttpApiGroup.make("group").add(
          HttpApiEndpoint.get("a", "/a", {
            success: Schema.String
          }).middleware(Effectful)
        )
      )
      const GroupLive = HttpApiBuilder.group(
        Api,
        "group",
        (handlers) => handlers.handle("a", (ctx) => Effect.succeed(ctx.request.headers["x-token"] ?? ""))
      )
      const EffectfulLive = Layer.succeed(Effectful, Effectful.of((httpEffect) => httpEffect))
      const EffectfulClient = HttpApiMiddleware.layerClient(
        Effectful,
        Effect.gen(function*() {
          const token = yield* Token
          return ({ next, request }) => next(HttpClientRequest.setHeader(request, "x-token", token))
        })
      ).pipe(Layer.provide(Layer.succeed(Token, "effect-token")))

      const ApiLive = HttpRouter.serve(
        HttpApiBuilder.layer(Api).pipe(Layer.provide(GroupLive), Layer.provide(EffectfulLive)),
        { disableListenLog: true, disableLogger: true }
      ).pipe(Layer.provideMerge(NodeHttpServer.layerTest))

      return Effect.gen(function*() {
        const client = yield* HttpApiClient.make(Api)
        yield* assertClientText(client.group.a().pipe(Effect.provide(EffectfulClient)), "effect-token")
      }).pipe(Effect.provide(ApiLive))
    })
    ...
    it.effect("security middleware participates in client middleware", () =>
      Effect.gen(function*() {
        const AuthorizationClient = HttpApiMiddleware.layerClient(Authorization, ({ next, request }) =>
          next(HttpClientRequest.setHeader(request, "cookie", "token=client-middleware")))
        const client = yield* HttpApiClient.make(Api)
        const user = yield* client.users.findById({ params: { id: -1 } }).pipe(
          Effect.provide(AuthorizationClient)
        )
        assert.strictEqual(user.name, "client-middleware")

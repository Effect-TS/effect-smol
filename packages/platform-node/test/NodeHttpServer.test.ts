...
describe("HttpServer", () => {
  it.effect("schema", () =>
    Effect.gen(function* () {
      ...
      const todo = yield* HttpClient.get("/todos/1").pipe(Effect.flatMap(HttpClientResponse.schemaBodyJson(Todo)))
  ...
  it.effect("formData withMaxFileSize", () =>
    Effect.gen(function* () {
      ...
      ).pipe(HttpRouter.serve, Layer.build, Effect.provideService(Multipart.MaxFileSize, 100))
      const client = yield* HttpClient.HttpClient
      const formData = new FormData()
  ...
  it.effect("formData withMaxFieldSize", () =>
    Effect.gen(function* () {
      ...
      ).pipe(HttpRouter.serve, Layer.build, Effect.provideService(Multipart.MaxFieldSize, 100))
      const client = yield* HttpClient.HttpClient
      const formData = new FormData()
  ...
  it.effect("html", () =>
    Effect.gen(function* () {
      yield* HttpRouter.addAll([
        ...
        HttpRouter.route("GET", "/about", HttpServerResponse.html`<html>${Effect.succeed("<body />")}</html>`),
        HttpRouter.route(
          "GET",
          "/stream",
  ...
  describe("HttpServerRespondable", () => {
    it.effect("error/schema", () =>
      Effect.gen(function* () {
        ...
        yield* HttpRouter.add("GET", "/home", new CustomError({ name: "test" }).asEffect()).pipe(
          HttpRouter.serve,
          Layer.build
        )
        const client = yield* HttpClient.HttpClient
  ...
  it.effect("RouterConfig", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(res.status, 204)
    }).pipe(Effect.provide([NodeHttpServer.layerTest, Layer.succeed(HttpRouter.RouterConfig)({ maxParamLength: 5 })]))
  )
  ...
  it.effect("HttpRouter prefixed", () =>
    Effect.gen(function* () {
      const handler = HttpRouter.serve(
        HttpRouter.use(
          Effect.fnUntraced(function* (router_) {
            ...
            yield* router.addAll([HttpRouter.route("GET", "/", Effect.succeed(HttpServerResponse.text("root")))])
          })
        )
      )
      ...
      const todo = yield* HttpClient.get("/todos/1").pipe(Effect.flatMap(HttpClientResponse.schemaBodyJson(Todo)))
      expect(todo).toEqual({ id: 1, title: "test" })
      const root = yield* HttpClient.get("/todos").pipe(Effect.flatMap((r) => r.text))

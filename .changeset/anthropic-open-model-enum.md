...
`Model` now generates as `Schema.Union([Schema.String, ...literals])`, matching how `@effect/ai-openai` already generates `ModelIdsShared`. Requests were unaffected and remain so: `AnthropicLanguageModel.layer`/`.model` already accepted any string.

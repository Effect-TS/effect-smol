...
Common `Struct` view projections also get fast paths for required-only, optional-only, and mutable-only field shapes, avoiding the fully generic optional-key and mutable-key reconstruction path when it is not needed.

On a medium-sized 48-entity domain-model smoke fixture reading `Type` views, total instantiations dropped from `861098` to `760908` (`-11.6%`). After subtracting the import baseline, the marginal domain-model cost dropped from `147842` to `101262` (`-31.5%`). Focused fixtures showed `Schema.StructWithRest` construction dropping from `66081` to `14124` delta instantiations (`-78.6%`) and a 200-field required-only `Struct.Type` projection dropping from `6653` to `2437` delta instantiations (`-63.4%`).

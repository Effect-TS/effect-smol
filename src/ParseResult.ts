...
export const flatMap: {
  <A, B, E1, R1>(
    f: (a: A) => Effect.Effect<B, E1, R1> | Either.Either<B, E1>
  ): <E, R>(
    self: Effect.Effect<A, E, R> | Either.Either<A, E>
  ) => Effect.Effect<B, E1 | E, R1 | R> | Either.Either<B, E | E1>
  <A, E, R, B, E1, R1>(
    self: Effect.Effect<A, E, R> | Either.Either<A, E>,
    f: (a: A) => Effect.Effect<B, E1, R1> | Either.Either<B, E1>
  ): Effect.Effect<B, E | E1, R | R1> | Either.Either<B, E | E1>
} = dual(2, <A, E, R, B, E1, R1>(
  self: Effect.Effect<A, E, R> | Either.Either<A, E>,
  f: (a: A) => Effect.Effect<B, E1, R1> | Either.Either<B, E1>
): Effect.Effect<B, E | E1, R | R1> | Either.Either<B, E | E1> => {
  if (isEither(self)) {
    return Either.match(self, { onLeft: Either.left, onRight: f })
  } else {
...
export const map: {
  <A, B>(
    f: (a: A) => B
  ): <E, R>(self: Effect.Effect<A, E, R> | Either.Either<A, E>) => Effect.Effect<B, E, R> | Either.Either<B, E>
  <A, E, R, B>(
    self: Effect.Effect<A, E, R> | Either.Either<A, E>,
    f: (a: A) => B
  ): Effect.Effect<B, E, R> | Either.Either<B, E>
} = dual(
  2,
  <A, E, R, B>(
    self: Effect.Effect<A, E, R> | Either.Either<A, E>,
...
export const mapError: {
  <E, E2>(
    f: (e: E) => E2
  ): <A, R>(self: Effect.Effect<A, E, R> | Either.Either<A, E>) => Effect.Effect<A, E2, R> | Either.Either<A, E2>
  <A, E, R, E2>(
    self: Effect.Effect<A, E, R> | Either.Either<A, E>,
    f: (e: E) => E2
  ): Effect.Effect<A, E2, R> | Either.Either<A, E2>
} = dual(
  2,
  <A, E, R, E2>(
    self: Effect.Effect<A, E, R> | Either.Either<A, E>,
...
export const mapBoth: {
  ...
  ): <R>(self: Effect.Effect<A, E, R> | Either.Either<A, E>) => Effect.Effect<A2, E2, R> | Either.Either<A2, E2>
  <A, E, R, E2, A2>(
    self: Effect.Effect<A, E, R> | Either.Either<A, E>,
    options: { readonly onFailure: (e: E) => E2; readonly onSuccess: (a: A) => A2 }
...
export const orElse: {
  <E, A2, E2, R2>(
    f: (e: E) => Effect.Effect<A2, E2, R2> | Either.Either<A2, E2>
  ): <A, R>(
    self: Effect.Effect<A, E, R> | Either.Either<A, E>
  ) => Effect.Effect<A2 | A, E2, R2 | R> | Either.Either<A | A2, E2>
  <A, E, R, A2, E2, R2>(
    self: Effect.Effect<A, E, R> | Either.Either<A, E>,
    f: (e: E) => Effect.Effect<A2, E2, R2> | Either.Either<A2, E2>
  ): Effect.Effect<A2 | A, E2, R2 | R> | Either.Either<A | A2, E2>
  ...
  if (isEither(self)) {
    return Either.match(self, { onLeft: f, onRight: Either.right })
  } else {
        ...
        if (queue && queue.length > 0) {
          ...
          return Effect.suspend(() => {
            ...
            return Effect.flatMap(
              ...
              () => Effect.fromEither(computeResult(state))
            )
          })
        }
              ...
              (state) =>
                Effect.suspend(() => {
                  if ("finalResult" in state) {
                    ...
                    return Effect.flatMap(isEither(pr) ? Effect.fromEither(pr) : Effect.either(pr), (t) => {
                      if (Either.isRight(t)) {
                        state.finalResult = t
                      } else {
...
const handleForbidden = <A, R>(
  effect: Effect.Effect<A, ParseIssue, R> | Either.Either<A, ParseIssue>,
  ast: AST.AST,
  actual: unknown,
  options: InternalOptions | undefined
): Effect.Effect<A, ParseIssue, R> | Either.Either<A, ParseIssue> => {
  ...
  if (exit) {
    ...
    return Either.left(new Forbidden(ast, actual, cause.failures.map(String).join("\n")))
  }
...
export const getFinalTransformation = (
  ...
  fromI: any
) => Effect.Effect<any, ParseIssue, any> | Either.Either<any, ParseIssue> => {
  switch (transformation._tag) {
    ...
    case "TypeLiteralTransformation":
      return (input) => {
        let out: Effect.Effect<any, ParseIssue, any> | Either.Either<any, ParseIssue> = Either.right(input)
